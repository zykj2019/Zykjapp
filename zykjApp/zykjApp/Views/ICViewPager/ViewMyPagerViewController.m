...
- (void)viewDidLoad {
    ...
    if (_scroll == 2) {
        ...
        [self.myContentView addGestureRecognizer:swipeGestureRecognizer];
...
- (void)viewWillLayoutSubviews {
    ...
    frame.origin.x = 0.0;
    frame.origin.y = self.tabLocation ? baseY + _tabTop : self.myContentView.frame.size.height - self.tabHeight - _tabTop;
    ...
    frame.size.width = self.myContentView.bounds.size.width;
    ...
    frame.size.width = self.myContentView.bounds.size.width;
    frame.size.height = self.tabLocation ? self.myContentView.frame.size.height - frame.origin.y : self.myContentView.frame.size.height -CGRectGetMinY(_tabsView.frame);
...
- (IBAction)scrollIndex:(NSInteger)index {
    ...
    [self.pageViewController setContentOffset:CGPointMake(index * self.myContentView.width, 0) animated:YES];
...
- (void)defaultSettings {
    ...
    [self.myContentView addSubview:_pageViewController];
...
- (void)reloadData {
    ...
    // Add tabsView
    CGFloat screenWidth = self.myContentView.frame.size.width;
    ...
    if (_customView) {
        ...
        [self.myContentView insertSubview:_customView atIndex:0];
    }
    ...
    [self.myContentView insertSubview:_tabsView atIndex:0];
    
    
    //bottomLine
    [_tabsBottomLine removeFromSuperview];
    _tabsBottomLine = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.myContentView.frame.size.width, 0.5)];
    ...
    // Add contentView
    _contentView = [self.myContentView viewWithTag:kPageViewTag];
    ...
    if (!_contentView) {
        ...
        _contentView.bounds = self.myContentView.bounds;
...
- (UIViewController *)viewControllerAtIndex:(NSUInteger)index {
    ...
    if ([[_contents objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        if ([self.dataSource respondsToSelector:@selector(viewPager:contentViewControllerForTabAtIndex:)]) {
            ...
            UIView *pageView = [self.myContentView viewWithTag:kPageViewTag];
