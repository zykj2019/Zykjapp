...
- (void)layoutSubviews {
    ...
    for (UIView *barButtonItemView in subviews) {
        ...
        @try {
            if ([NSStringFromClass(barButtonItemView.class) isEqualToString:@"_UIToolbarContentView"]) {
                NSLayoutConstraint *c = [barButtonItemView valueForKey:@"_buttonBarLeadingConstraint"];
                if (c.constant < 0 || c.constant == 0) {
                    c.constant = 8;
                }
                
                c = [barButtonItemView valueForKey:@"_buttonBarTrailingConstraint"];
                if (c.constant  > 0 || c.constant == 0) {
                    c.constant = -8;
                }
            }
