...
{
    if (self.reader) {
        NSLog(@"exportAsynchronouslyWithCompletionHandlered");
        _error = [NSError errorWithDomain:AVFoundationErrorDomain code:AVErrorExportFailed userInfo:@
                  {
        NSLocalizedDescriptionKey: @"exportAsynchronouslyWithCompletionHandlered"
        }];
        [self cancelExport];
        handler();
        return;
    ...
    if (videoTracks.count > 0) {
        ...
        [self.videoInput requestMediaDataWhenReadyOnQueue:self.inputQueue usingBlock:^
         {
            //            dispatch_group_enter(self.inputDispatchGroup);
            if (![wself encodeReadySamplesFromOutput:wself.videoOutput toInput:wself.videoInput])
            {
                @synchronized(wself)
                {
                    ...
                    //                     dispatch_group_leave(self.inputDispatchGroup);
...
- (BOOL)encodeReadySamplesFromOutput:(AVAssetReaderOutput *)output toInput:(AVAssetWriterInput *)input
{
    while (input.isReadyForMoreMediaData)
    {
        ...
        if (sampleBuffer)
        {
            NSLog(@"fileout:%@____%@",self.outputURL.lastPathComponent,input);
            BOOL handled = NO;
            BOOL error = NO;
