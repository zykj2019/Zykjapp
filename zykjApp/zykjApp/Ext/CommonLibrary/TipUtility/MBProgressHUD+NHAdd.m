...
NS_INLINE MBProgressHUD *createNew(UIView *view) {
    ...
    MBProgressHUD *hd = [[MBProgressHUD alloc] initWithView:view];
    [view addSubview:hd];
    return hd;
}
...
+ (MBProgressHUD *)showOnlyTextToViewNoHidden:(UIView *)view title:(NSString *)title {
     return settHUD(view, title, NO);
}
...
+ (void)showTitleToView:(UIView *)view
           contentStyle:(NHHUDContentStyle)contentStyle
                  title:(NSString *)title
             afterDelay:(NSTimeInterval)delay completion:(void (^)(void))completion {
    MBProgressHUD *hud = settHUD(view, title, NO);
    hud.mode = MBProgressHUDModeText;
    hud.hudContentStyle(contentStyle);
    [hud hideAnimated:YES afterDelay:delay];
