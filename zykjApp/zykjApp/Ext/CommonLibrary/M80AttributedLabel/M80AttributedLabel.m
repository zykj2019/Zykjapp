...
@interface M80AttributedLabel ()
{
    ...
    CGFloat                     _fontHeight;
    M80AttributedLabelURL      * _ttouchedLink;  //搭配isCanMove
    BOOL                     isActivateLongTap;
}
...
@property (assign, nonatomic) BOOL isAllHighlight;
@end
...
- (void)setLongTapTime:(CGFloat)longTapTime {
    self.canClick = longTapTime > 0 ? YES : NO;
    _longTapTime = longTapTime;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (drawString)
    {
        ...
        [self drawHighlightAllWithRect:rect];
...
- (void)drawHighlightAllWithRect:(CGRect)rect {
    if (self.isAllHighlight && self.highlightColor) {
        CGContextRef context = UIGraphicsGetCurrentContext();
        CGContextAddRect(context,rect);//填充框
        [[self.highlightColor colorWithAlphaComponent:0.8] setFill];
...
- (void)setImgSize{
    ...
    [_imgSizes removeAllObjects];
    NSAttributedString *drawString = [self attributedStringForDraw];
    //    NSLog(@"NSAttributedString:%@",drawString.string);
    if (drawString) {
        CGFloat height = CGRectGetHeight(self.bounds);
        [self prepareTextFrame:drawString rect:self.bounds];
        CFArrayRef lines = CTFrameGetLines(_textFrame);
        CFIndex lineCount = CFArrayGetCount(lines);
        CGPoint lineOrigins[lineCount];
        CTFrameGetLineOrigins(_textFrame, CFRangeMake(0, 0), lineOrigins);
        ...
        for (CFIndex i = 0; i < numberOfLines; i++)
        {
            ...
            for (CFIndex k = 0; k < runCount; k++)
            {
                ...
                id content = attributedImage.content;
                if ([content isKindOfClass:[UIImage class]])
                {    attatchmentRect.origin.y = height - attatchmentRect.origin.y - attatchmentRect.size.height;
                    [self.imgSizes addObject:[NSValue valueWithCGRect:attatchmentRect]];
                }
                else
                {
                    NSLog(@"Attachment Content Not Supported %@",content);
...
- (BOOL)onLabelClick:(CGPoint)point
{
    ...
    if (linkData)
    {
        if (_delegate && [_delegate respondsToSelector:@selector(m80AttributedLabel:clickedOnLink:)] && !isActivateLongTap)
        {
            if (_longTapTime) {
                [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(tabLongHandle) object:nil];
            }
    ...
    if (_ttouchedLink) {
        if (_delegate && [_delegate respondsToSelector:@selector(m80AttributedLabel:clickedOnLink:)] && !isActivateLongTap)
        {
            ...
            [_delegate m80AttributedLabel:self clickedOnLink:_ttouchedLink.linkData];
        }
        _ttouchedLink = nil;
        return YES;
    }
               ...
               sync:(BOOL)sync
{
    ...
    LinkBlock block = ^(NSArray *links)
    {
        ...
        if (self.dataDelegate && [self.dataDelegate respondsToSelector:@selector(saveM80Links:)]) {
            [self.dataDelegate saveM80Links:links];
        }
    ...
    if (self.dataDelegate && [self.dataDelegate respondsToSelector:@selector(m80Links)]) {
        NSArray *links = [self.dataDelegate m80Links];
        if (links) {
            block(links);
            return;
        }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    //记录原始链接
    _ttouchedLink  = self.isCanMove ? self.touchedLink : nil;
    if (self.touchedLink)
    {
        [self setNeedsDisplay];
    }
    ...
    if (_longTapTime) {
        isActivateLongTap = NO;
        [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(tabLongHandle) object:nil];
        [self performSelector:@selector(tabLongHandle) withObject:nil afterDelay:_longTapTime];
    }
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.touchedLink || self.isAllHighlight)
    {
        self.touchedLink = nil;
        self.isAllHighlight = NO;
        [self setNeedsDisplay];
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if(![self onLabelClick:point])
    {
        if (_delegate && [_delegate respondsToSelector:@selector(m80AttributedLabelAndClickedOnSelf:)] && !isActivateLongTap)
        {
            if (_longTapTime) {
                [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(tabLongHandle) object:nil];
            }
            
            [_delegate m80AttributedLabelAndClickedOnSelf:self];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (touchedLink == nil)
    {
        ...
        if (_canClick && [super pointInside:point withEvent:event]) {
            return self;
        }
        return nil;
    }
...
- (void)tabLongHandle {
    NSLog(@"tabLongHandle");
    isActivateLongTap = YES;
    if (_delegate && [_delegate respondsToSelector:@selector(m80AttributedLabelAndLongClickedOnSelf:)]) {
        [_delegate m80AttributedLabelAndLongClickedOnSelf:self];
    }
}
@end
