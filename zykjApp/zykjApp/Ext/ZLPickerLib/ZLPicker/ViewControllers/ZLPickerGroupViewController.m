...
- (void) jump2StatusVc{
    ...
    for (ZLPickerGroup *group in self.groups) {
        if ((self.status == PickerViewShowStatusCameraRoll || self.status == PickerViewShowStatusPhotoVideo) && ([group.groupName isEqualToString:@"Camera Roll"] || [group.groupName isEqualToString:@"相机胶卷"] || [group.groupName isEqualToString:@"所有照片"])) {
            gp = group;
            break;
            ...
            gp = group;
            break;
        } else if (self.status == PickerViewShowStatusVideo) {
            if ([group.groupName isEqualToString:@"所有照片"]) {
                 gp = group;
                break;
            }
        }
...
-(void)getImgs{
    ...
    switch (self.status) {
        case PickerViewShowStatusPhotoVideo:
            datas.assetsFilter = [ALAssetsFilter allAssets];
            break;
        case PickerViewShowStatusVideo:
            datas.assetsFilter = [ALAssetsFilter allVideos];
            break;
            
        default:
             datas.assetsFilter = [ALAssetsFilter allPhotos];
            break;
    }
//    datas.assetsFilter = self.status == PickerViewShowStatusPhotoVideo ? [ALAssetsFilter allAssets] : [ALAssetsFilter allPhotos];
