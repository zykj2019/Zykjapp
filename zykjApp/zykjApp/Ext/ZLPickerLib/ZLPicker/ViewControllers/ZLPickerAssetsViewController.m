...
//原图btn
@property (nonatomic , strong) UIButton *orginSendBtn;
...
- (UIButton *)orginSendBtn {
    if (_orginSendBtn == nil && [ZLPickerGlobal sharedInstance].isShowOriginalFileSend) {
        UIButton *orginSendBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [orginSendBtn setImage:[UIImage imageNamed:@"zlpicker_btn_normal"] forState:UIControlStateNormal];
        [orginSendBtn setImage:[UIImage imageNamed:@"zlpicker_btn_press"] forState:UIControlStateSelected];
        orginSendBtn.titleLabel.font = [UIFont systemFontOfSize:15.0];
        [orginSendBtn setTitleColor:[UIColor colorWithRed:0/255.0 green:91/255.0 blue:255/255.0 alpha:1.0] forState:UIControlStateNormal];
        [orginSendBtn setTitleColor:[UIColor grayColor] forState:UIControlStateDisabled];
        [orginSendBtn setTitle:@"原图" forState:UIControlStateNormal];
        [orginSendBtn addTarget:self action:@selector(orginSendAction:) forControlEvents:UIControlEventTouchUpInside];
        [orginSendBtn sizeToFit];
        _orginSendBtn = orginSendBtn;
    }
    return _orginSendBtn;
}
...
//        toolBarThumbCollectionView.translatesAutoresizingMaskIntoConstraints = NO;
        ...
        CGFloat width = [UIScreen mainScreen].bounds.size.width - 60.0 - self.doneBtn.width - self.orginSendBtn.width;
        self.toolBarThumbCollectionView.frame = CGRectMake(0, 0, width, 44);
        
//        [self.toolBar addSubview:toolBarThumbCollectionView];
//        NSDictionary *views = NSDictionaryOfVariableBindings(toolBarThumbCollectionView, _doneBtn);
//
...
- (void) setupToorBar{
    ...
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:heightVfl options:0 metrics:0 views:views]];
     ...
     // 左视图 中间距 右视图
       ...
       NSMutableArray *items = @[leftItem,fiexItem].mutableCopy;
       if (self.orginSendBtn) {
           [items addObject:[[UIBarButtonItem alloc] initWithCustomView:self.orginSendBtn]];
...
- (void)orginSendAction:(UIButton *)sender {
    sender.selected = !sender.isSelected;
    [ZLPickerGlobal sharedInstance].isOriginalFileSend = sender.isSelected;
}
@end
