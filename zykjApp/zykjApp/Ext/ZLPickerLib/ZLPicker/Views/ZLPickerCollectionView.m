...
@interface ZLPickerCollectionView () <UICollectionViewDataSource,UICollectionViewDelegate,AVPlayerViewControllerDelegate>
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cellImgView.selectBlock = ^{
        [ws clickCollectionView:collectionView didSelectItemAtIndexPath:indexPath];
    };
    ...
    if ([asset isKindOfClass:[ALAsset class]]) {
        cellImgView.videoDuration = [asset videoDuration];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self clickCellCollectionView:collectionView didSelectItemAtIndexPath:indexPath];
}
...
//点击选择图标
- (void)clickCollectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (pickerImageView.isMaskViewFlag) {
        ...
        if (self.selectsIndexPath.count >= minCount) {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提醒" message:[NSString stringWithFormat:@"最多只能选择%zd个资源文件",minCount] delegate:self cancelButtonTitle:nil otherButtonTitles:@"好的", nil];
            [alertView show];
            return ;
        }
...
//点击cell
- (void)clickCellCollectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ALAsset *asset = self.dataArray[indexPath.row];
    if (asset.isVideo) {
        ...
        id next = [self nextResponder];while(![next isKindOfClass:[UIViewController class]]) {
            next = [next nextResponder];
        };
        
        [ ((UIViewController *)next).navigationController presentViewController:playerViewController animated:YES completion:^{
