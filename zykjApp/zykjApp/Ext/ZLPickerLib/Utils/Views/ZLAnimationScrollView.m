...
+ (instancetype)animationViewWithOptions:(NSDictionary *)options animations:(void (^)(void))animations completion:(void (^)(ZLAnimationBaseView *))completion{
    ...
    if ([self currentPage] < subViews.count) {
        if (tableView || collectionView) {
            ...
            if (direction == UICollectionViewScrollDirectionHorizontal) {
                ...
                // 竖屏
                startFrame = ((UIView *)subViews[[self currentPage]]).frame;
