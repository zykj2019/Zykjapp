...
#pragma mark - 开始动画
+ (instancetype) animationViewWithOptions:(NSDictionary *)options animations:(void(^)(void))animations completion:(void (^)(ZLAnimationBaseView *baseView)) completion{
...
+ (void) restoreAnimation:(void(^)(void)) completion{
...
+ (void) restoreWithOptions:(NSDictionary *)options animation:(void (^)(void))completion{
...
//            return [_options[UIViewAnimationInView] frame];
            return ((UIView *)_options[UIViewAnimationInView]).frame; 
        }else{
            return CGRectZero;
        }
    }
