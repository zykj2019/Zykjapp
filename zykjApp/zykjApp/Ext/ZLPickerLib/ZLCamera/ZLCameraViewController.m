...
@interface ZLCameraViewController () <UIActionSheetDelegate,UICollectionViewDataSource,UICollectionViewDelegate,AVCaptureMetadataOutputObjectsDelegate,ZLCameraImageViewDelegate,ZLCameraViewDelegate,ZLPickerViewControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
-(void)Captureimage
{
    ...
    [self.captureOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         ...
         //         t_image = [self scaleImage:t_image toScale:0.5f];
...
- (void)configGlobal {
    //初始化
    [ZLPickerGlobal sharedInstance].isShowOriginalFileSend = self.isShowOriginalFileSend;
    [ZLPickerGlobal sharedInstance].isOriginalFileSend = NO;
}
...
//        [self.selectAssets[indexPath.item] thumbnail
        asset = (ALAsset *)obj;
        if ([asset isKindOfClass:[ALAsset class]]) {
             image = [UIImage imageWithCGImage:[[asset defaultRepresentation] fullScreenImage]];
            if (!image) {
    ...
    if (parseAssetComplate) {
        parseAssetComplate(obj,image,asset);
    }
}
//开始拍照1
- (void)openCamera {
...
-(void)LocalPhoto
{
    [self LocalPhotoWithStatus:PickerViewShowStatusCameraRoll];
}
...
-(void)LocalPhotoWithStatus:(PickerViewShowStatus)status {
    ...
    if (author ==kCLAuthorizationStatusRestricted || author ==kCLAuthorizationStatusDenied){
        ...
        // 没有权限
        [HUDHelper alertTitle:@"提示" message:@"请在设备的\"设置-隐私-照片\"中允许访问照片。" cancel:@"确定"];
        return;
    ...
    //    pickerVc.minCount = 9;
    pickerVc.minCount = self.minCount;
    pickerVc.status = status;
...
{
    if (self.images.count >= self.minCount)
    {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提醒" message:[NSString stringWithFormat:@"最多只能添加%ld个资源文件",(long)self.minCount] delegate:self cancelButtonTitle:nil otherButtonTitles:@"好的", nil];
        [alertView show];
